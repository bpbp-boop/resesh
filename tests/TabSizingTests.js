...
test("each tab bar keeps compact current-folder and file-pane buttons at its far right", () => {
  ...
  assert.match(strip, /<ColumnDefinition Width="\*" \/>[\s\S]*?<ColumnDefinition Width="72" \/>/);
  ...
  assert.match(currentFolder, /Glyph="&#xE8B7;"[\s\S]*?FontSize="14"/);
  assert.match(currentFolder, /Glyph="&#xE72C;"[\s\S]*?FontSize="8"/);
...
test("the tab-bar toggle follows selection and all file-pane open and close routes", () => {
  ...
  assert.match(code, /FilePaneOpenChanged \+= FilePaneButtonView_FilePaneOpenChanged/);
